...
class DigitCaps(nn.Module):
    ...
    def __init__(self, in_dim, in_caps, out_caps, out_dim, num_routing):
        ...
        self.in_caps = in_caps  #32*6*6=1152
        self.out_caps = out_caps #10
    ...
    def forward(self, x):
        ...
        for route_iter in range(self.num_routing - 1):
            ...
            s = (c * temp_u_hat).sum(dim=2)  #中间胶囊  用权重c衡量浅层胶囊对深层胶囊的重要程度，是动态变化的，故动态路由
            ...
            uv = torch.matmul(temp_u_hat, v.unsqueeze(-1))  #更新权重b, 考虑浅层和深层胶囊的输出，若他们相似，则同向，值大，反之则小
            b += uv
